...
def plot_results(preds, labels, images, epoch, batch_index , batch_size, dir_path, wandb_logger, test=False):
    ...
    for k in range(batch_size):    
        ...
        axarr[0,k].set_title("Real",fontsize=28)
        axarr[0,k].axis("off")
        axarr[1,k].imshow(preds[k].cpu().numpy(), cmap="gray")
        axarr[1,k].set_title("Output",fontsize=28)
        axarr[1,k].axis("off")
        axarr[2,k].imshow(labels[k].cpu().numpy(), cmap="gray")
        axarr[2,k].set_title("True",fontsize=28)
        axarr[2,k].axis("off")
    ...
    plt.savefig(dir_path + f"/plots/plots_{string}.png",transparent=True,bbox_inches='tight')
