...
class ResNet(nn.Module):
    def __init__(self, cfg):
        ...
        conv_layers = [nn.Conv2d(self.params.n_in, self.params.n_features, kernel_size=3, stride=1, padding=1),
                       nn.ReLU()]
        for i in range(self.params.num_res_blocks):
            conv_layers.append(ResNetBlock(self.params.n_features))
        self.res_blocks = nn.Sequential(*conv_layers)
        self.fc = nn.Sequential(nn.Linear(32*32*self.params.n_features, 2048),
                                nn.ReLU(),
                                nn.Linear(2048, 512),
                                nn.ReLU(),
